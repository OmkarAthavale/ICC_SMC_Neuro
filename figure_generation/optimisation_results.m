...
for i = 1:4
    hold on;
    scatter(i+0.2+(rand(1, size(sol_plot, 1))*0.2), sol_plot(:, i), 4, 'k', 'filled')
end
...
100.*(quantile(icc.fval(low_mask), 0.25)),...
100.*(quantile(icc.fval(low_mask), 0.75)));
fprintf('\n')
...
100.*(quantile(icc.fval(high_mask), 0.25)),...
100.*(quantile(icc.fval(high_mask), 0.75)));
fprintf('\n')
